...
public class OrderAdminController {
    ...
    public String exportFillToExcel(HttpServletResponse response, @ModelAttribute DateFilterDTO dateParam) throws IOException, IllegalStateException{
        response.setContentType("application/octet-stream");
        DateFormat dateFormat= new SimpleDateFormat("yyyy-MM-dd_HH");
        String currentDateTime = dateFormat.format(new Date());
        String headerKey = "Content-Disposition";
        ...
        String headerValue = "attachement; filename= "+fileName;
        response.setHeader(headerKey,headerValue);
        List<OrderAdmin> orderAdmins = adminService.getOrderAdminByFillter(dateParam.getStartFill(),dateParam.getEndFill());
        OrderDataExcelExport orderDataExcelExport = new OrderDataExcelExport(orderAdmins);
        System.err.println(orderAdmins.toString());
        orderDataExcelExport.export(response);
